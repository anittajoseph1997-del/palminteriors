...
const AIDesignStudio: React.FC = () => {
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      // Create history for the API, excluding the current message as it's passed separately or as the last item
      const history = chatMessages.map(m => ({ 
        role: m.role === 'model' ? 'model' : 'user', 
        parts: [{ text: m.text }] 
      }));
      
      const response = await getDesignAdvice(userText, history);
      setChatMessages(prev => [...prev, { role: 'model', text: response || 'I could not generate a response. Please try again.' }]);
    } catch (error) {
      setChatMessages(prev => [...prev, { role: 'model', text: 'An error occurred while connecting to Palm Studio. Please check your connection.' }]);
    } finally {
      setIsTyping(false);
    }
  };
  ...
  const handleGenerateConcept = async (e: React.FormEvent) => {
    ...
    try {
      const imageUrl = await generateRoomConcept(userPrompt);
      if (imageUrl) {
        ...
        alert('Could not generate the image. Please try a different prompt.');
      }
    } catch (error) {
      console.error(error);
      alert('Generation failed. Please ensure your prompt is descriptive.');
    } finally {
          ...
          <div className="mt-auto">
             <div className="p-6 bg-stone-900 rounded-3xl text-white">
                ...
                <div className="flex items-center">
                  <span className={`w-2 h-2 rounded-full mr-3 ${isTyping || isGenerating ? 'bg-yellow-400 animate-pulse' : 'bg-green-400'}`}></span>
                  <span className="text-xs font-semibold">{isTyping || isGenerating ? 'Processing...' : 'Ready for input'}</span>
                </div>
             </div>
          </div>
        </div>
              ...
              <div className="flex-1 overflow-y-auto p-10 space-y-8">
                {chatMessages.map((msg, idx) => (
                  <div key={idx} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                    <div className={`max-w-[85%] p-6 rounded-3xl ${
                      msg.role === 'user' 
                      ? 'bg-stone-900 text-white rounded-tr-none shadow-lg' 
                      : 'bg-stone-100 text-stone-800 rounded-tl-none font-light border border-stone-200'
                    }`}>
                ...
                {isTyping && (
                  <div className="flex justify-start">
                    <div className="bg-stone-100 p-6 rounded-3xl rounded-tl-none border border-stone-200">
              ...
              <div className="p-8 border-t border-stone-100">
                <form onSubmit={handleSendMessage} className="flex space-x-4">
                  <input
                    ...
                    placeholder="Ask about materials, layout, or Kochi manufacturing..."
                    className="flex-1 px-8 py-5 rounded-full bg-stone-50 border border-stone-100 focus:ring-2 focus:ring-stone-900 outline-none transition-all text-sm"
                  />
                  <button 
                    type="submit"
                 ...
                 <form onSubmit={handleGenerateConcept} className="space-y-6">
                   <div className="relative">
                      <textarea
                        ...
                        className="w-full px-8 py-6 rounded-[32px] bg-stone-50 border border-stone-100 focus:ring-2 focus:ring-stone-900 outline-none transition-all resize-none h-32 text-sm leading-relaxed"
                      />
                   </div>
                   <button 
                    type="submit"
                         ...
                         <div key={c.id} className="bg-white rounded-[32px] overflow-hidden shadow-2xl border border-stone-100 group">
                           <div className="relative aspect-video">
                              ...
                              <div className="absolute inset-0 bg-stone-900/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                                <a 
                                  href={c.imageUrl} 
                                  download={`palm-concept-${c.id}.png`}
                                  className="bg-white text-stone-900 px-8 py-3 rounded-full text-xs font-bold uppercase tracking-widest shadow-2xl scale-90 group-hover:scale-100 transition-all"
                                >
                                   Save Concept
                                </a>
                              </div>
